		...
		: m_Spec(spec)
	{
		if (!WasGLFWInit)
		{
			if (glfwInit() == GLFW_FALSE)
				MANGO_STOP("Failed to initialize GLFW! Something is really wrong...");

			glfwSetErrorCallback([](int error, const char* description)
				{
					MANGO_LOG_ERROR("GLFW error! " + std::to_string(error) + ": " + description);
				});
		}

		glfwWindowHint(GLFW_MAXIMIZED, spec.Maximized);
